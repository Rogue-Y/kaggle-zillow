...
# Parameter spaces for ensembling:
space_lightgbm = {
    'model_params': {
        'learning_rate': hp.loguniform('learning_rate', -2, 0),
        'boosting_type': 'gbdt',
...
space_xgb = {
    'model_params': {
        ...
        'min_child_weight': hp.uniform('min_child_weight', 0.1, 5),
        'subsample': hp.choice('subsample', [x/10 for x in range(5, 11)]),
        # 'colsample_bytree': hp.choice('colsample_bytree', [x/10 for x in range(3, 8)]),
        # 'colsample_bylevel': hp.choice('colsample_bylevel', [x/10 for x in range(1, 7)]),
        'lambda': hp.choice('lambda', [x/10 for x in range(3, 8)]),
        'alpha': hp.choice('alpha', [x/10 for x in range(0, 5)]),
...
space_rf = {
    'model_params': {
        'n_estimators': hp.choice('n_estimators', list(range(10, 36, 5))),
        'criterion': hp.choice('criterion', ['mae', 'mse']),
        'max_features': hp.uniform('max_features', 0.5, 1),
        'max_depth': hp.choice('max_depth', [None, *list(range(1, 6))]),
        'min_samples_split': hp.choice('min_samples_split', [3, 10, 30, 100, 300]),
        'min_samples_leaf': hp.choice('min_samples_leaf', [30, 70, 100, 150, 300]),
        'n_jobs': -1
    },
...
experiments = [
    ...
    (Lightgbm.Lightgbm, configuration['feature_list'], space_lightgbm, 1000, {}),
    (RFRegressor.RFRegressor, configuration['feature_list'], space_rf, 1000, {'clean_na': True}),
]
...
def tune_stacking():
    for Model, feature_list, parameter_space, max_evals, exp_params in experiments:
        tune_single_stacking(Model, parameter_space, max_evals, exp_params)

def tune_single_stacking(Model, parameter_space, max_evals, exp_params, trials=None):
    def train_wrapper(params):
        meta_model = Model(model_params=params['model_params'])
        loss = stacking(meta_model=meta_model)
        # return an object to be recorded in hyperopt trials for future uses
        return {
            'loss': loss,
            'status': STATUS_OK,
    ...
    # save the experiment trials in a pickle
    folder = 'data/trials/ensemble'
    if not os.path.exists(folder):
        os.makedirs(folder)
    with open("%s/%s_%s_pickle" %(folder, Model.__name__, timestamp), "wb") as trial_file:
